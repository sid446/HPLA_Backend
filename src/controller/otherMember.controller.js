...
const UpdateOtherMember = asyncHandler(async (req, res) => {
    ...
    if (existingMember) {
        throw new ApiError(409, "Member with this membership number or mobile already exists");
    }
...
const deleteOtherMember = asyncHandler(async (req, res) => {
    ...
    return res.status(200).json(
        new ApiResponse(200, null, "Member deleted successfully")
    );
});
export {SaveOtherMember,UpdateOtherMember,getAllOtherMembers,deleteOtherMember}