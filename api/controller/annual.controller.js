...
const createAnnualReport =  AsyncHandler(async (req, res) => {
    ...
    if (!annualReport) {
        throw new ApiError(500, "Something went wrong while saving the annual report");
    }
...
const editAnnualReport=AsyncHandler(async(req,res)=>{
   ...
   if(!id || !title || !link) {
    ...
    const updatedReport = await AnnualReport.findByIdAndUpdate(id, { title, link }, { new: true });
    if (!updatedReport) {
        throw new ApiError(500, "Failed to update annual report");
    }
    return res.status(200).json(new ApiResponse(200, updatedReport, "Annual report updated successfully"));
})

export { createAnnualReport, getAnnualReport, deleteAnnualReport ,editAnnualReport};